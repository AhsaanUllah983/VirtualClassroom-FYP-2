...
#     return render(request, 'vc/insights.html')
...
# cap = cv2.VideoCapture (0)
    ...
    while True:
        ...
        if key%256 == 27:
            ...
            print(faceMatch)
            print(faceNotMatch)
            dat = datetime.datetime.now().strftime("%m/%d/%Y")
            savercrd = Attendance()
            savercrd.studentId =studentId
...
def video_feed(request):
    ...
    # x=()
    # print(x)
    # for num in gazeTrack():
    #     # print("1")
    #     print(num)
        # return HttpResponse("1")
        # return HttpResponse(num, content_type='multipart/x-mixed-replace; boundary=frame')
    for students in Student.objects.raw('SELECT studentId,email,password,uname FROM student'):
        if((students.email == request.session.get('email')) and ((students.password == request.session.get('password')))):
            studentId=students.studentId
